...
import { CountUp } from "@/components/v2/motion";
...
const KAIRO = "#3b82f6";
/* Neutral rather than a second brand colour: this bar is "not Kairo",
   which is a category, not a value judgement — and a warm accent here
   would read as a second brand rather than as data. */
const OTHER = "#94a3b8";
  ...
  return (
    <div className="grid gap-px border border-line bg-line lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      ...
      <div className="bg-ground p-7 sm:p-10">
        <p className="chrome mb-8">Your business</p>
        ...
        <div className="border-t border-line pt-6">
          ...
          <p className="mt-4 text-[12px] leading-relaxed text-ink-3">
            Both are scenarios you switch on yourself, priced at your own ticket. Turn them off and
            the visits figure still stands on its own.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-between bg-raised p-7 sm:p-10">
        <div>
          ...
          {/* The payoff figure of the whole section, so it arrives rather
              than simply being there. It tracks the sliders after that. */}
          <p className="tabular display text-[clamp(52px,7vw,104px)] leading-[0.84]">
            <CountUp value={r.visits} />
          </p>
          <p className="mt-3 text-[17px] font-semibold">{r.visits === 1 ? "visit" : "visits"}.</p>

          <p className="mt-4 max-w-[38ch] text-[13.5px] leading-relaxed text-ink-2">
            {money(SETUP)} to set up, once. At {money(ticket)} a visit that's {r.visits}{" "}
            {r.visits === 1 ? "client" : "clients"} through the chair — then nothing per month, for
        ...
        <div className="mt-10">
          <div className="border border-line-2 bg-ground p-5">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-ink-2">
              No subscription, no per-booking fee, no commission, no cut of what you charge.
          ...
          <a href="/get-in-touch" className="btn-brand mt-4 w-full">
            Contact us
          </a>
        </div>
      </div>
  ...
  return (
    <Reveal>
      <p className="mt-6 max-w-3xl text-[12px] leading-relaxed text-ink-3">
        The only figures asserted here are Kairo's own: {money(SETUP)} to set up, and nothing per
        month. The ticket price, the monthly fee and the two scenarios are yours to set, and the
        arithmetic is plain — we don't estimate what another platform costs you, because that
        depends on terms we can't see.
      </p>
    </Reveal>
  );
}
