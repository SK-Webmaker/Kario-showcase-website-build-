...
export function Hero() {
  ...
  /* How much of the scroll the mark is currently obeying, 0 to 1.
     It used to be a boolean: hold everything at zero until the intro
     finished, then hand over. Anyone who scrolled during that second and a
     half saw nothing move, then watched the mark jump to wherever they had
     got to — the jolt this replaces. Now it ramps, so an early scroll is
     caught up to smoothly instead of arriving all at once. */
  const [gate, setGate] = useState(reduced ? 1 : 0);
  /* Set when the visitor scrolls before the intro is done. The strokes
     finish drawing fast instead of making them wait out the full sequence:
     the intro is a courtesy, not a toll. */
  ...
  useEffect(() => {
    ...
    const startRaf = requestAnimationFrame(() => setDrawing(true));
    ...
    const ramp = () => {
      ...
      const tick = (now: number) => {
        const t = clamp01((now - t0) / 420);
        setGate(easeInOut(t));
        if (t < 1) rampRaf = requestAnimationFrame(tick);
      };
      rampRaf = requestAnimationFrame(tick);
    };

    const onEarlyScroll = () => {
      if (window.scrollY < 2) return; // ignore the browser restoring position
    ...
    // 1750ms is when the last stroke's draw transition actually ends
    // (1200ms duration behind a 550ms delay), so the handover lands on the
    // frame the drawing finishes rather than 150ms of dead air later.
  ...
  /* The rotation starts almost immediately. It used to wait until 16% of
     the section's travel — about forty vertical percent of scrolling that
     moved nothing at all, which reads as a page that has not loaded. */
  const tRot = gate * easeInOut(phase(p, 0.05, 0.3));
  const tPos = gate * easeInOut(phase(p, 0.14, 0.44));
  const tFill = gate * easeInOut(phase(p, 0.26, 0.54));
    ...
    <section ref={ref} id="top" className="relative h-[260vh]">
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div
          ...
          style={{
            opacity: stage,
            transform: `scale(${lerp(1, 0.95, tHandoff)})`,
            pointerEvents: "none",
          }}
        >
          <svg
                ...
                <line
                  ...
                  style={{
                    transition: reduced
                      ? "none"
                      : rushed
                        ? "stroke-dashoffset 260ms cubic-bezier(.33,1,.68,1), stroke 700ms ease"
        ...
        {/* Corner meta. Fills the frame the way an instrument panel does,
            and carries the three facts an owner wants in the first
          ...
          <div className="shell grid grid-cols-3 gap-8">
            ...
            <div>
              <p className="chrome-2 max-w-[42ch] normal-case tracking-normal leading-relaxed">
                Kairo is the diary, the client book, the till and the messages, on your own booking
                link — no marketplace, no commission, and nothing shared with another business.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <p
            ...
            style={{
              animation: reduced ? "none" : "rise-in 900ms cubic-bezier(.66,0,.01,1) 900ms both",
            }}
          >
            Kairo
          </p>
          <p
            ...
            style={{
              animation: reduced ? "none" : "rise-in 900ms cubic-bezier(.66,0,.01,1) 1150ms both",
            }}
          >
            The software a business runs its whole day on
          </p>
          <div
            className="chrome mt-9 flex items-center gap-2"
            style={{
              animation: reduced ? "none" : "rise-in 900ms cubic-bezier(.66,0,.01,1) 1500ms both",
            }}
          >
            Scroll
            <svg
            ...
            >
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                ...
                <a href="#money" className="btn-line">
                  See it find lost money
                </a>
              </div>
              <p className="chrome mt-5 text-center normal-case tracking-normal text-ink-2">
                Ask a question, book a demo, or tell us you're ready — we reply the same day.
              </p>
            </div>
